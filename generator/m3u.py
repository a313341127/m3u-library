...
# ---------------------------------------------------------------- 标题清洗
# 注意：不能用 \b 词边界 —— 中文字符在 Python re 里也算「词字符」，所以
# 「美国往事4K」中「事」与「4」之间没有词边界，\b 会导致 4K 匹配失败、
# 清晰度标记留在片名里，同一部片裂成「美国往事」+「美国往事4K」两张卡
# （实测全库 movie 有 602 行如此）。改用「相邻字符不是字母数字」判断，
# 既能在中文旁正常剥离，又不会误伤 HDMI/UHD/DiscoveryHD 这类词。
# 守卫必须覆盖拉丁扩展字母（U+00C0–U+024F）：否则《…nähdä…》里的 hd 会被误删。
_ALNUM = r"A-Za-z0-9\u00c0-\u024f"
_TITLE_CLEAN_RE = re.compile(
    r"(?i)(?<![" + _ALNUM + r"])(1080p|720p|2160p|4k|hd|bd)(?![" + _ALNUM + r"])"
...
def clean_title(name: str) -> str:
    ...
    # 清理残留括号/分隔符（下划线是采集站给清晰度标记加的占位符，如「xxx_1080P_」）
    n = re.sub(r"[\[\]【】()（）・·…_]", " ", n)
